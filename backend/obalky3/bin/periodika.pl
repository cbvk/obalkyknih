...
while(<DATA>) {
	my($rok,$c1,$c2,$nazev) = split(/\s*\@\s*/);
	# http://www.nkp.cz/tituly/images/2001/12/47.jpg
	my $url = "http://www.nkp.cz/tituly/images/$rok/$c1/$c2.jpg";
	my($foo,$sysno) = /(request|number)\=(\d+)/ or next;
#	warn "Duplicitni $sysno\n" if($url{$sysno}++);
	$url{$sysno} = $url;
}
...
open(SEQ,"<utf8","periodika.dat") or die;
while(<SEQ>) {
	if(/^(\d+)\s022\s+L\s\$\$[ay](\d{4}\-?\d\d\d\w)$/) {
		$issn{int($1)} = $2;
	}
	if(/^(\d+)\s015\s+L\s\$\$a(cnb\d+)$/) {
		$cnb{int($1)} = $2;
	}
# 001181295 001   L cps20021181295
# 001181295 015   L $$acnb001181295
# 001181295 022   L $$a1213-919X
}
close(SEQ);
...
foreach my $sysno (sort keys %url) {
	my $issn = $issn{int($sysno)};
	my $cnb = $cnb{int($sysno)};
	warn "No ISSN neither CNB for $sysno\n" unless($issn or $cnb);
	my $id = join("_", grep $_, ($cnb || "",($issn ? "issn$issn" : "")));

	next unless(-f "/opt/obalky/periodika/$id.jpg");
	
	my $product_url = $url{$sysno};
	my $bibinfo = Obalky::BibInfo->new_from_params(
						{ nbn => $cnb, issn => $issn });
	my $media =  Obalky::Media->new_from_info({ cover_url => $product_url, 
				  cover_tmpfile => "/opt/obalky/periodika/$id.jpg" });
	my $product = $eshop->add_product($bibinfo,$media,$product_url);
	print "added $id (".$product->id.")\n";

#	print "wget -O $id.jpg ".$url{$sysno}."\n" if($id);
}

#	print "wget -O $sysno.jpg $url\n"
