...
foreach my $eshop (@eshops) {
	...
	foreach(@list) {
		...
		if ($product) {
			if ($pdfurl) {
				# pridaj fulltext PDF
				DB->resultset('ProductParams')->find_or_create({ product=>$product, book=>$product->book, other_param_type=>3, other_param_value=>$pdfurl });
				# odmaz iny fulltext PDF = nastala zmena URL
				my $fullTextPdf = DB->resultset('ProductParams')->search({ product=>$product->id, book=>$product->book->id, other_param_type=>3 });
				foreach ($fullTextPdf->next) {
					$_->delete if ($_->get_column('other_param_value') ne $pdfurl);
				}
			} else {
				# odmaz, protoze v krameriovi uz neexistuje
				my $fullTextPdf = DB->resultset('ProductParams')->search({ product=>$product->id, book=>$product->book->id, other_param_type=>3 });
				$fullTextPdf->delete if ($fullTextPdf->count);
			}
		}
