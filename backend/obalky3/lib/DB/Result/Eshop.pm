...
# DO NOT MODIFY THIS OR ANYTHING ABOVE! md5sum:Wf6pHELZZAabpuomcnKoEw
...
sub add_product {
	...
	# zamka se pri skenovani klientem odemyka, aby DB trigger nebranil aktualizaci
	my ($book_pomocna, $eshop_id) = (undef, undef); 
	if ($book) {
		$book_pomocna = $book->get_column('pomocna');
		$eshop_id = $eshop->get_column('id');
		if ($book_pomocna == 1 and $eshop_id == 109) {
			$book->update({ pomocna => undef });
		}
	}
	...
	# opetovne uzamceni docasne odemceneho zaznamu pri skenovani klientem
	if ($book and $book_pomocna == 1 and $eshop_id == 109) {
		$book->update({ pomocna => $book_pomocna });
	}
