...
#$ean13 = '978-80-249-3417-4';
	if ($ean13 || $nbn){
		...
		if ($nbn){
#		        $query_nbn = '@or ' . $query_nbn if ($query_nbn eq '');
                        $nbn =~ s/\s//g;
...
#die;
	...
	for (my $i=0; $i<$size; $i++) {
		...
		$title1 = $marc->title();
		$title2 = $bibinfo->{title};
		# shoda prochazenim titulu 1 - MARC
		my @words = split / /, $title1;
		$matches = $total = 0;
		foreach $word (@words) {
		        next if length($word) < 4;
		        $word =~ s/\,$//g; $word =~ s/\.$//g; $word =~ s/\)$//g; $word =~ s/^\(//g; $word =~ s/\/$//g;
		        $total++;
                        next if index($title2, $word) == -1;
                        $matches++;
		}
                $found = 1 if ($matches/$total)>0.6;
                # shoda prochazenim titulu 2 - obalky
                @words = split / /, $title2;
		$matches = $total = 0;
		foreach $word (@words) {
		        next if length($word) < 4;
		        $word =~ s/\,$//g; $word =~ s/\.$//g; $word =~ s/\)$//g; $word =~ s/^\(//g; $word =~ s/\/$//g;
		        $total++;
                        next if index($title1, $word) == -1;
                        $matches++;
		}
                $found = 1 if ($total>0 and ($matches/$total)>0.4);
		#$found = 1 if (index($title1WoSpace, $title2WoSpace) gt -1 and index($title1WoSpace, $title2WoSpace) lt 20);
warn '===> FOUND <===' if $found;
warn '===> NOT FOUND <===' unless $found;
	}
