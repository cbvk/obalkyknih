...
sub blue_stash {
	...
	if(not $blue_timestamp or time - $blue_timestamp > 60*60) {
		...
		$blue_info{covers} = DB->resultset('TmpPerf')->find('cover_cnt')->get_column('val');
		$blue_info{tocs} = DB->resultset('TmpPerf')->find('toc_cnt')->get_column('val');
		$blue_info{recent} = DB->resultset('Product')->recent(16)
...
sub view : Local {
	...
	foreach (keys %count) {push (@bound_ids, $_) if ($count{$_} eq $relation_type_count); warn $count{$_}; warn $relation_type_count; }
	#foreach (@found_ids) {push @bound_ids, $_};
