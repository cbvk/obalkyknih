...
sub save_to_hash {
	...
	delete $hash->{authors} if ($hash->{authors} eq '');
	return $hash;
}
...
sub identifiers {
	...
	if (defined $bibinfo->{bookid}) {
		...
		foreach my $rec ($res->all) {
			...
			push @recs, $newBibinfo if ($newBibibfo);
			my $newBibinfo2 = Obalky::BibInfo->new_from_params({
				ean13 => $rec->ean13,
				nbn => $rec->nbn,
				oclc => $rec->oclc
			}, undef);
			push @recs, $newBibinfo2 if ($newBibibfo2);
		}
	}
	
	# identifikatory z tabulky ident
	my $resIdent = DB->resultset('Ident')->search({ book => $bibinfo->{bookid}, 'deny' => 0 });
	while (my $row = $resIdent->next) {
		my $type = $row->get_column('type');
		my $val = $row->get_column('val');
		my $newBibinfo3 = Obalky::BibInfo->new_from_params({
			ean13 => $type==1 ? $val : undef,
			nbn => $type==3 ? $val : undef,
			oclc => $type==2 ? $val : undef
		}, undef);
		push @recs, $newBibinfo3;
	}
