...
my $r = Redis->new( server => 'localhost:6380', encoding => 'UTF-8' );
my $res = $r->hkeys('rel:other');
my $i=0;
map {
    my $rel;
    my $relCount = 0;
    my $val = $r->hget('rel:other', $_);
    my @edArr = split(/#/, $val);
    ...
    if ($rel and $relCount gt 1) {
        ...
        map {
            ...
            # dopln text edicie z tagu 787g
            my $bookUpd = DB->resultset('Book')->find($idParent);
            $bookUpd->update({ 'other_relation_text' => $edText }) if ($bookUpd);
        } keys %{$rel};
    }
