...
# posledny sklizeny zaznam
my $maxRes = DB->resultset('Auth')->search(undef, {
    columns => [
        { maxDateStamp => { max => "oai_datestamp" } }
    ]
});
my $maxRet = $maxRes->next;

my $records = $harvester->listRecords(
	'metadataPrefix'  => 'marc21',
	'set'             => 'AUT',
	'from'            => $maxRet->get_column('maxDateStamp'),
	'until'           => DateTime->now->ymd.'T'.DateTime->now->hms.'Z',
	'metadataHandler' => 'MarcOAI'
);
	...
	while ( my $record = $records->next() ) {
		...
		if (defined $metadata->{marc}) {
			...
			if ($metadata->{marc} and $metadata->{marc}->field('100') and $metadata->{marc}->field('001')) {
				...
				next if (substr($rec->{'id'},0,3) eq 'aun');
