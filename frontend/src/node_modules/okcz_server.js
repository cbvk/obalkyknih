...
var server = function (req, response, db)
{
   ...
   **/
    ...
    if (!sigla) {
      ...
      response.end('Unknown referer. You need to sign up at http://www.obalkyknih.cz and provide your catalog URL');
      return;
    }
   ...
   **/
    ...
    if (!query.isbn && !query.nbn && !query.oclc && !query.multi) {
    ...
    db.collection(metaCollection).find(sel).toArray(function (err, items) {
      ...
      if (!items.length) {
        ...
        // v pripade dotazu pomoci multi identifikatoru muzeme dostat vic zaznamu
        // nutne rozhodnout podle priority, kterou obalku zobrazit
        var item=null;
        var flag_root_or_standalone_found = false; // priznak toho, jestli jsem pri prochazeni zaznamy narazily na part_root, nebo part_*_standalone
        for (var i=0; i<3; i++) { // i==0:ISBN/i==1:OCLC/i==2:NBN
          if ((i==0 && !bib.isbn) || (i==1 && !bib.oclc) || (i==2 && !bib.nbn)) continue; // nepokracuj u tohoto identifikatoru pokud jej dotaz bibinfo neobsahuje
          for (var j=0; j<items.length; j++) {
            var itemTmp=items[j];
            
            // hledani vyskytu rozhodne sekce (part_root, nebo part_*_standalone)
            if (itemTmp.part_root=='1' || (i==0 && itemTmp.part_ean_standalone=='1') || (i==1 && itemTmp.part_oclc_standalone=='1') || (i==2 && itemTmp.part_nbn_standalone=='1'))
               flag_root_or_standalone_found = true;
            
            // pokud metazaznam neobsahuje obrazek, nepokracuj
            var itemImgExists=(itemTmp.cover_medium_url || itemTmp.cover_thumbnail_url || itemTmp.cover_icon_url);
            if (!itemImgExists) continue;
            
            // pokud metazaznam neobsahuje pozadovane identifikatory
            if (!(
                // shoda podle roku + cisla u periodika
                (bib.part_year && bib.part_no && itemTmp.part_year==bib.part_year && itemTmp.part_no==bib.part_no) || 
                // shoda podle rocniku + cisla u periodika
                (bib.part_volume && bib.part_no && itemTmp.part_volume==bib.part_volume && itemTmp.part_no==bib.part_no) || 
                // shoda podle cisla/nazvu castu u monografii
                ((bib.part_no || bib.part_name) && !bib.part_year && !bib.part_volume && (itemTmp.part_no==bib.part_no || itemTmp.part_name==bib.part_name)) ||
                // nehledame zaznam podle zadneho part_* identifikatoru
                (!bib.part_no && !bib.part_name)
            )) continue;
            
            if (i==0) { // ISBN
              if (itemTmp.ean!=bib.isbn) continue;
              if (itemTmp.part_root=='0' && itemTmp.part_ean_standalone=='0' && !bib.part_no && !bib.part_name) continue;
              item=itemTmp;
            } else if (i==1) { // OCLC
              if (itemTmp.oclc!=bib.oclc) continue;
              if (itemTmp.part_root=='0' && itemTmp.part_oclc_standalone=='0' && !bib.part_no && !bib.part_name) continue;
              item=itemTmp;
            } else if (i==2) { // NBN
              if (itemTmp.nbn!=bib.nbn) continue;
              if (itemTmp.part_root=='0' && itemTmp.part_nbn_standalone=='0' && !bib.part_no && !bib.part_name) continue;
              item=itemTmp;
            }
        ...
        // pokud se ptame na souborny zaznam, nebo monografii, v DB byl nejaky zaznam, ale zadny "prioritny" s part_root/part_*_standalone, musime se dotazat na zaslani souborneho zaznamu BE
        if (!item && !bib.part_no && !bib.part_name && !flag_root_or_standalone_found) {
          metadata([{bibinfo:bib,permalink:"a"}],[],true,false,false);
          return;
        }
      }
      ...
      if (!items.length) {
        ...
        for (var i=0; i<3; i++) { // i==0:ISBN/i==1:OCLC/i==2:NBN
          ...
          for (var j=0; j<items.length; j++) {
            ...
            // pokud metazaznam neobsahuje obrazek, nepokracuj
            if (!itemTmp.toc_thumbnail_url) continue;
      ...
      if (!items.length) {
        ...
        for (var i=0; i<3; i++) { // i==0:ISBN/i==1:OCLC/i==2:NBN
          ...
          for (var j=0; j<items.length; j++) {
            ...
            // pokud metazaznam neobsahuje obrazek, nepokracuj
            if (!itemTmp.toc_pdf_url) continue;
   ...
   **/
    ...
    db.collection(metaCollection).find({$or:[{ean:{$in:isbna}},{nbn:{$in:nbna}},{oclc:{$in:oclca}}]}).sort({part_root:-1.0}).toArray(function (err, items) {
    ...
    if (items)
      for (var i=0; i<items.length; i++) {
        ...
        var part_root=item.part_root || '0';
        var part_ean_standalone=item.part_ean_standalone || '0';
        var part_nbn_standalone=item.part_nbn_standalone || '0';
        var part_oclc_standalone=item.part_oclc_standalone || '0';
          ...
          for (var j=0;j<3;j++) {
            ...
            for (k in fetcha) {
              ...
              if ( !(
                  ...
                  (part_year && part_no && bibinfo.part_year && bibinfo.part_no && part_year==bibinfo.part_year && part_no==bibinfo.part_no) || 
                  // periodikum podle rocniku a cisla
                  (part_volume && part_no && bibinfo.part_volume && bibinfo.part_no && part_volume==bibinfo.part_volume && part_no==bibinfo.part_no) || 
                  // monografie podle cisla casti
                  (part_no && bibinfo.part_no && !bibinfo.part_year && !bibinfo.part_volume && part_no==bibinfo.part_no) ||
                  // monografie podle nazvu casti
                  (part_name && bibinfo.part_name && !bibinfo.part_year && !bibinfo.part_volume && part_name==bibinfo.part_name) ||
                  // souborny zaznam, nebo cast monografie bez dotazu na konkretni cast
                  (!bibinfo.part_no && !bibinfo.part_name) ))
              {
                continue;
              }
              ...
              if (j==0 && bibinfo.isbn) {
                ...
                if (!isbnBib || !ean) continue;
                if (isbnBib!=ean) continue;
                if (part_root=='0' && part_ean_standalone=='0' && !bibinfo.part_no && !bibinfo.part_name) continue;
                found=true;
              } else if (j==1 && bibinfo.oclc) {
                // OCLC
                if (!bibinfo.oclc || !oclc) continue;
                if (bibinfo.oclc!=oclc) continue;
                if (part_root=='0' && part_oclc_standalone=='0' && !bibinfo.part_no && !bibinfo.part_name) continue;
                found=true;
              } else if (j==2 && bibinfo.nbn) {
                // NBN
                if (!bibinfo.nbn || !nbn) continue;
                if (bibinfo.nbn!=nbn) continue;
                if (part_root=='0' && part_nbn_standalone=='0' && !bibinfo.part_no && !bibinfo.part_name) continue;
                found=true;
              }
              
              if (found) {
                item.bibinfo=bibinfo.bibinfo;
   ...
   **/
    ...
    if (perms[remoteIP]) {
      ...
      if (query.book_id) {
        console.log(JSON.stringify(query.book_id));
        sel.$or.push({book_id : query.book_id});
        sel.$or.push({book_id_parent : query.book_id});
      }
      if (sel.$or.length == 0) {
        response.writeHead(404);
        ...
        if (!error) {
          ...
          for (var i=0;i<jsonf.length;i++) {
            ...
            if (!item.bibinfo || !fetchaStr.length) {
              ...
              var pos=fetchaStr.indexOf(getSortedBibinfo(item.bibinfo));
              if (pos>=0) {
                json[pos]=replaceUrl(item, bibinfoOrig[pos]);
                delete fetchaStr[pos];
              } else
                json.push(replaceUrl(item));
            }
