...
//var tocFileDir = "c:/Apps/nodejs/toc/"
...
var server = function (req, response, db)
{
  ...
  if (etags[etag] !== undefined) {
    var sigla=refererValid(referer,remoteIP);
   ...
   **/
    ...
    if (query.isbn) {
      ...
      if (multis.isbn) { sel.$or.push({ean:multis.isbn}); bib.isbn=multis.isbn; }
        ...
        if (items.length==1) {
          ...
          for (var i=0; i<3; i++) {
            for (var j=0; j<items.length; j++) {
              ...
              if (i==0) { // ISBN
                if (itemTmp.ean==bib.isbn) var item=itemTmp;
              } else if (i==1) { // OCLC
                if (itemTmp.oclc==bib.oclc) var item=itemTmp;
              } else if (i==2) { // NBN
                if (itemTmp.nbn==bib.nbn) var item=itemTmp;
              }
   ...
   **/
    ...
    if (perms[remoteIP]) {
      if (query.isbn) {
        ...
        var sel={nbn : query.nbn};
      } else if (query.oclc) {
        var sel={oclc : query.oclc};
      } else {
        response.writeHead(404);
      ...
      db.collection(metaCollection).findOne(sel, function (err, item) {
        if (!item) {
          response.end('not found');
          return;
        }
        // console.dir(item);
        var sel={$or:[{url:getUrlByType(item)},{url:getUrlByType(item,'icon')},{url:getUrlByType(item,'thumbnail')},{url:getUrlByType(item,'toc_thumbnail')}]}
        var toc=item.toc_thumbnail_url
        var tocPdf=item.toc_pdf_url
        // vymaz i TOC nahled, pokud nejaky je
        if (toc) {
          sel.$or.push({toc_thumbnail_url:toc.slice(toc.indexOf('/',8))});
        ...
        /// asynchronous
        db.collection('cover').find(sel).toArray(function (err, items) {
          if (!items) return;
          
          for (var i=0; i<items.length; i++) {
            var cid=items[i]._id;
            var item_type=(items[i].url.indexOf(urlTocFile)>0)?'toc_thumbnail':'cover';
            ...
            })(cid, item_type, items[i].url);
          }
        });

        /// asynchronous
        db.collection(metaCollection).remove({_id:item._id},{single:true}, function(err, removed){
      ...
      req.on('end', function () {
        ...
        if (book_id > 0) {
          ...
          if (review_count != parseInt(query.review_count)) {
            response.writeHead(404);
            response.end('review count michmach');
            return;
          }
...
function getUrlByType(metadata, type) {
  ...
  url = url || metadata.cover_medium_url || "medium";
