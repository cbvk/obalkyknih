...
var urlReplace = "cache.obalkyknih.cz"; //cache1
...
var ipBackend = "::ffff:172.30.144.13"; //cache1
...
var urlBibFile = "file/bib";
var urlBibApiThumbnail = "api/bib/thumbnail";
var urlBibApiPdf = "api/bib/pdf";
var uriPartBibThumbnail = "/thumbnail";
var uriPartBibPdf = "/pdf";
var urlDoporucApi = "api/doporuc";
...
var doporucCollection = "okcz_doporuc";
...
var bibFileDir = "/home/users/okcz/static/bib/";
// autority
...
var Statistics = /** @class */ (function () {
    ...
    Statistics.addEtagBibPdfMatch = function (sigla) {
        this.etagBibPdfMatch[sigla]++ || (this.etagBibPdfMatch[sigla] = 1);
        if (sigla != 'admin')
            this.etagBibPdfMatch.admin++ || (this.etagBibPdfMatch.admin = 1);
    };
    Statistics.addEtagBibThumbMatch = function (sigla) {
        this.etagBibThumbMatch[sigla]++ || (this.etagBibThumbMatch[sigla] = 1);
        if (sigla != 'admin')
            this.etagBibThumbMatch.admin++ || (this.etagBibThumbMatch.admin = 1);
    };
    ...
    Statistics.addBibThumbnailRequests = function (sigla) {
        this.bibThumbnailRequests[sigla]++ || (this.bibThumbnailRequests[sigla] = 1);
        if (sigla != 'admin')
            this.bibThumbnailRequests.admin++ || (this.bibThumbnailRequests.admin = 1);
    };
    Statistics.addBibPdfRequests = function (sigla) {
        this.bibPdfRequests[sigla]++ || (this.bibPdfRequests[sigla] = 1);
        if (sigla != 'admin')
            this.bibPdfRequests.admin++ || (this.bibPdfRequests.admin = 1);
    };
    ...
    Statistics.addBibThumbnailApiRequests = function (sigla) {
        this.bibThumbnailApiRequests[sigla]++ || (this.bibThumbnailApiRequests[sigla] = 1);
        if (sigla != 'admin')
            this.bibThumbnailApiRequests.admin++ || (this.bibThumbnailApiRequests.admin = 1);
    };
    Statistics.addBibPdfApiRequests = function (sigla) {
        this.bibPdfApiRequests[sigla]++ || (this.bibPdfApiRequests[sigla] = 1);
        if (sigla != 'admin')
            this.bibPdfApiRequests.admin++ || (this.bibPdfApiRequests.admin = 1);
    };
    ...
    Statistics.addBibThumbnailRemoves = function () {
    ...
    Statistics.addBibThumbnailNotfound = function (sigla) {
        this.bibThumbnailNotfound[sigla]++ || (this.bibThumbnailNotfound[sigla] = 1);
        if (sigla != 'admin')
            this.bibThumbnailNotfound.admin++ || (this.bibThumbnailNotfound.admin = 1);
    };
    Statistics.addBibPdfNotfound = function (sigla) {
        this.bibPdfNotfound[sigla]++ || (this.bibPdfNotfound[sigla] = 1);
        if (sigla != 'admin')
            this.bibPdfNotfound.admin++ || (this.bibPdfNotfound.admin = 1);
    };
    ...
    Statistics.addBibThumbnailFetches = function (sigla) {
        this.bibThumbnailFetches[sigla]++ || (this.bibThumbnailFetches[sigla] = 1);
        if (sigla != 'admin')
            this.bibThumbnailFetches.admin++ || (this.bibThumbnailFetches.admin = 1);
    };
    Statistics.addApiAliveRequest = function () {
        this.apiRuntimeRequests++;
    };
    Statistics.addUnboundRequest = function () {
        this.unboundRequests++;
    };
    Statistics.addOperativeRequest = function () {
    ...
    Statistics.startLivemap = function () {
        this.livemap = {};
    };
    Statistics.stopLivemap = function () {
        this.livemap = null;
    };
    Statistics.getLivemap = function () {
        return this.livemap;
    };
    ...
    Statistics.etagBibPdfMatch = {}; // pocet dotazu na BIB PDF, kesovanych prohlizecem od spusteni
    Statistics.etagBibThumbMatch = {}; // pocet dotazu na BIB nahled, kesovanych prohlizecem od spusteni
    ...
    Statistics.bibThumbnailRequests = {}; // pocet dotazu BIB nahledu pres urlBibFile thumbnail (/file/toc/xxxxxx/thumbnail) od spusteni
    Statistics.bibThumbnailApiRequests = {}; // pocet dotazu BIB nahledu pres urlBibApi thumbnail od spusteni
    Statistics.bibThumbnailFetches = {}; // pocet nacteni BIB nahledu z backendu od spusteni
    Statistics.bibThumbnailNotfound = {}; // pocet nenalezenych BIB nahledu z backendu od spusteni
    Statistics.bibThumbnailRemoves = 0; // pocet smazanych BIB nahledu z backendu od spusteni
    Statistics.bibPdfRequests = {}; // pocet dotazu na BIB dokumenty pres urlBibFile thumbnail (/file/bib/xxxxxx/thumbnail) od spusteni
    Statistics.bibPdfApiRequests = {}; // pocet dotazu na BIB dokumenty pres urlBibApiThumbnail od spusteni
    Statistics.bibPdfNotfound = {}; // pocet nenalezenych na BIB dokumenty z backendu od spusteni
    ...
    Statistics.apiRuntimeRequests = 0; // pocet dotazu na /api/tuntime/alive
    Statistics.unboundRequests = 0; // vsechny jine dotazy
    Statistics.operativeRequests = 0; // operativni dotazy (pridani opravneni, komentare/hodnoceni, vsechny ridici requesty mezi BE a FE)
    Statistics.dateTimeout = 0; // cas do kdy je platny timeout
    Statistics.dateStartup = new Date(); // doba startu
    Statistics.livemap = null; // fe live mapy
    return Statistics;
}());
// ====================== PERMISSIONS.TS ===================
var Permissions = /** @class */ (function () {
    ...
    Permissions.getSiglaForHeader = function (headers) {
        // user agent
        ...
        if (!headers['x-library-client'])
            return;
        var libraryClient = headers['x-library-client'];
        var librarySigla = headers['x-library-code'] || '';
        if (libraryClient.substring(0, 5) == 'KPSYS') {
            return '_KPSYS';
        }
        else if (libraryClient.substring(0, 7).toUpperCase() == 'tritius') {
            if (librarySigla.length != 6)
                librarySigla = 'TRI001';
    ...
    Permissions.refererValid = function (referer, ip, encsigla, headers, query) {
        var sigla;
        if (encsigla) {
    ...
    Permissions.ipValid = function (xreferer, ip, sigla, headers) {
        //console.log('ipValid:' + ip);
        var tmp = Permissions.refererValid(xreferer, ip, null, headers, null);
        if (!tmp)
            return false;
        if (tmp === 'sigla') {
            return sigla;
...
var Helpers = /** @class */ (function () {
    ...
    Helpers.replaceUrl = function (meta, simple, noocr, bibinfo) {
    ...
    Helpers.getUrlByType = function (metadata, type) {
        ...
        switch (type) {
            ...
            case 'bib_thumbnail':
                ...
                if (!url)
                    return null; // bib_thumbnail nesmi v pripade neexistence vratit cover_medium_url
                break;
            default:
...
// ==========================================================
var PushSettingsRecord = /** @class */ (function () {
...
var Server = /** @class */ (function () {
    ...
    Server.prototype.go = function (s) {
                    ...
                    for (var i = 0; i < items.length; i++) {
                        ...
                        if (itemInfo.item !== null && s.isGoodItemInfoForBibInfo(itemInfo, bibinfo)) {
                            ...
                            for (var j = 0; j <= 7; j++) {
                                var found = s.itemFound(itemInfo, bibinfo, j);
                                if (found) {
                                    var bibThumbnailUrl = Helpers.getUrlByType(itemInfo.item, 'bib_thumbnail');
        ...
        // http://cache1.obalkyknih.cz/api/books/?multi=[{%22isbn%22:%229788376663103%22},{%22isbn%22:%229788376663203%22}]&sigla=CBA001&pretty=1
            ...
            var sigla = Permissions.ipValid(this.xreferer || this.referer, this.remoteIP, this.query.sigla, this.headers);
            ...
            var fetchaAll_1 = Object.assign({}, fetcha);
            var dbFind = [];
            if (isbna.length) {
            ...
            this.db.collection(metaCollection).find({ $or: dbFind }).toArray(function (err, items_raw) {
                            ...
                            for (var j = 0; j <= 7; j++) {
                                ...
                                for (var k in fetcha) {
                                    ...
                                    if (found) {
                                        ...
                                        var itemToInsert = Object.assign({}, Helpers.replaceUrl(itemInfo.item, resSimple, resNoocr));
                                        if (pos >= 0) {
                ...
                s.metadata(s, fetchaBE, fetchaAll_1, json);
            });
        }
        /**
             * AUTH METADATA
        ...
        // http://cache1.obalkyknih.cz/api/auth/meta/?id=jk01083016&sigla=CBA001&pretty=1
            ...
            var sigla = Permissions.ipValid(this.xreferer, this.remoteIP, this.query.sigla, this.headers);
         ...
         **/
            ...
            this.db.collection(metaCollection).find(sel).toArray(function (err, items) {
                ...
                for (var i = 0; i < items.length; i++) {
                    var item = items[i];
                    var bookId = item.book_id;
                    // dotaz do kolekce cover; hledame obalky a nahledy obsahu na smazani
                    var subSel = item.cover_medium_url ? { $or: [{ url: Helpers.getUrlByType(item) }, { url: Helpers.getUrlByType(item, 'icon') }, { url: Helpers.getUrlByType(item, 'thumbnail') }, { url: Helpers.getUrlByType(item, 'preview510') }, { url: Helpers.getUrlByType(item, 'toc_thumbnail') }, { url: Helpers.getUrlByType(item, 'bib_thumbnail') }] } : { $or: [] };
                    // TOC
                    ...
                    // BIB
                    if (item.bib_thumbnail_url)
                        subSel.$or.push({ url: Helpers.getUrlByType(item, 'bib_thumbnail') });
                    var bib = item.bib_thumbnail_url;
                    var bibPdf = item.bib_pdf_url;
                    ...
                    s.db.collection('cover').find(subSel).toArray(function (err, items) {
                        ...
                        for (var j = 0; j < items.length; j++) {
                            var cid = items[j]._id;
                            var item_type = 'cover';
                            if (items[j].url.indexOf(urlTocFile) > 0)
                                item_type = 'toc_thumbnail';
                            if (items[j].url.indexOf(urlBibFile) > 0)
                                item_type = 'bib_thumbnail';
            ...
            if (s.remoteIP === ipBackend) {
                ...
                s.db.collection(metaAuthCollection).find(sel).toArray(function (err, items) {
                    ...
                    for (var i = 0; i < items.length; i++) {
                        var item = items[i];
                        // dotaz do kolekce cover; hledame obalky a nahledy obsahu na smazani
                        var subSel = item.cover_medium_url ? { $or: [{ url: Helpers.getUrlByType(item) }, { url: Helpers.getUrlByType(item, 'icon') }, { url: Helpers.getUrlByType(item, 'thumbnail') }] } : { $or: [] };
                        /// asynchronous
                        s.db.collection('cover').find(subSel).toArray(function (err, items) {
                            if (!items)
                                return;
         ...
         **/
            ...
            if (this.req.method == 'POST') {
                ...
                this.req.on('end', function () {
                    ...
                    var get_data_db = { book_id: s.query.book_id, sigla: sigla };
                    if (s.query.id)
                        get_data_db.id = s.query.id;
         ...
         **/
            ...
            if (this.req.method == 'POST') {
                ...
                this.req.on('end', function () {
                    var book_id = parseInt(s.query.book_id);
                    if (book_id > 0) {
                        try {
    ...
    Server.prototype.metadata = function (s, fetcha, fetchaAll, json, coverApi, tocThumbApi, tocPdfApi, bibThumbApi, bibPdfApi) {
            ...
            request({ url: url, timeout: timeout }, function (error, res, body) {
                if (!error) {
                    ...
                    for (var i = 0; i < jsonf.length; i++) {
                        item = jsonf[i];
                        var recId = (item.ean || '#') + '-' + (item.oclc || '#') + '-' + (item.nbn || '#') + '-' + (item.ismn || '#');
                ...
                if (coverApi) {
                    ...
                    var item = json[0];
                    var bibThumbnailUrl = Helpers.getUrlByType(item, 'bib_thumbnail');
                    if (s.sendPlaceholderIfNotValue(bibThumbnailUrl)) {
                        return;
                    }
                    ...
                    var item = json[0];
                    var bibPdfUrl = item.bib_pdf_url;
                ...
                if (query.doporuc && query.doporuc == '1') {
                    (function () {
                        return __awaiter(this, void 0, void 0, function () {
                            var i, item, itemdb, urlOkczdObj, urlOkczdUrl, itemInsert, dataOkczd, restmp;
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0:
                                        i = 0;
                                        _a.label = 1;
                                    case 1:
                                        if (!(i < json.length)) return [3 /*break*/, 8];
                                        item = json[i];
                                        if (!item || !item.book_id)
                                            return [3 /*break*/, 7];
                                        return [4 /*yield*/, db.collection(doporucCollection).findOne({ 'book_id': item.book_id })];
                                    case 2:
                                        itemdb = _a.sent();
                                        if (itemdb && itemdb.notfound < now)
                                            itemdb = undefined;
                                        if (!!itemdb) return [3 /*break*/, 6];
                                        urlOkczdObj = fetchaAll[i];
                                        delete urlOkczdObj.bibinfo;
                                        urlOkczdUrl = JSON.stringify(urlOkczdObj);
                                        itemInsert = {};
                                        return [4 /*yield*/, s.getData('http://okczd.obalkyknih.cz/api/doporuc?multi=' + urlOkczdUrl)];
                                    case 3:
                                        dataOkczd = _a.sent();
                                        if (!dataOkczd.length) {
                                        ...
                                        return [4 /*yield*/, db.collection(doporucCollection).deleteMany({ 'book_id': item.book_id })];
                                    case 4:
                                        _a.sent();
                                        return [4 /*yield*/, db.collection(doporucCollection).insertOne(itemInsert)];
                                    case 5:
                                        _a.sent();
                                        _a.label = 6;
                                    case 6:
                                    ...
                                    case 7:
                                        ...
                                        return [3 /*break*/, 1];
                                    case 8:
                                        restmp = JSON.stringify(json, null, query.pretty ? ' ' : '');
                                        response.end(restmp);
                                        return [2 /*return*/];
                                }
                            });
    ...
    Server.prototype.cover = function (s, coverUrl, coverApi, tocThumbApi, bibThumbApi) {
        var sigla = s.sigla;
        var req = s.req;
        var db = s.db;
        var response = s.response;
        ...
        db.collection('cover').findOne({ url: coverUrl }, function (err, item) {
            ...
            if (tocThumbApi || (coverUrl.indexOf(urlTocFile)) > 0)
                coverType = 'toc';
            if (bibThumbApi || (coverUrl.indexOf(urlBibFile)) > 0)
                coverType = 'bib';
    ...
    // poskytnuti souboru s TOC
    Server.prototype.bib = function (s, reqUrl) {
        var sigla = s.sigla;
        var req = s.req;
        var db = s.db;
        var response = s.response;
        ...
        testLog("43", "!!!![metadata] sigla: " + sigla + ", req: " + req + ", db: " + db + ", response: " + response + ", remoteIP:" + remoteIP + ", referer: " + referer + ", timestamp: " + timestamp);
        reqUrl = reqUrl.replace('http://' + urlMain, '');
        var bibPdfUrlSegments = reqUrl.split('/');
        var bibFileName = bibPdfUrlSegments[bibPdfUrlSegments.length - 2];
        var bibExtension = bibPdfUrlSegments[bibPdfUrlSegments.length - 1];
        if (!bibFileName) {
            s.send404IfNotValue(false);
        }
        var mimeType = 'application/' + (bibExtension || 'pdf');
        var bibFileEncapDir = Math.ceil((parseInt(bibFileName) + 1) / 10000) * 10000;
        var bibFilePath = bibFileDir + bibFileEncapDir + '/' + bibFileName + '.' + (bibExtension || 'pdf');
        fileSystem.exists(bibFilePath, function (exists) {
            if (exists) {
                var bibFileStat = fileSystem.statSync(bibFilePath);
                ...
                var readStream = fileSystem.createReadStream(bibFilePath);
                readStream.pipe(response);
    ...
    /* VLASTNE -------------------------------------------------------------------------------------------------- */
    Server.prototype.logUnbound = function (s) {
        var toLog = {
            'date': s.date,
            'requrl': s.requrl,
            'remoteIP': s.remoteIP,
    ...
    Server.prototype.getResult = function () {
        ...
        if (this.query.isbn || this.query.ean) {
            ...
            var sel = { $or: [{ nbn: value }, { nbn_other: value }] };
            var bib = { nbn: value };
            return (!value) ? undefined : { bib: bib, sel: sel };
        }
            ...
            var sel = { $or: [{ oclc: value }, { oclc_other: value }] };
            var bib = { oclc: value };
            return (!value) ? undefined : { bib: bib, sel: sel };
        }
    ...
    Server.prototype.itemFound = function (itemInfo, bibinfo, j) {
        if (j == 0 && bibinfo.isbn) {
            ...
            if (itemInfo.ean_other)
                ean_other_length = itemInfo.ean_other.length;
            ...
            // OTHER NBN
            var nbn_other_length = 0;
            if (itemInfo.nbn_other)
                nbn_other_length = itemInfo.nbn_other.length;
            for (var i = 0; i < nbn_other_length; i++) {
            ...
            // OTHER OCLC
            var oclc_other_length = 0;
            if (itemInfo.oclc_other)
                oclc_other_length = itemInfo.oclc_other.length;
            for (var i = 0; i < oclc_other_length; i++) {
