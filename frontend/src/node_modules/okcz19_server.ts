...
var urlReplace: string = "192.168.1.110"        //local
...
var ipBackend: string = "192.168.1.110"         //local

var urlPart: string = "/"
var urlMetadata: string = "api/books"
...
var metaCollection: string = "okcz"
         ...
         **/
            ...
            this.db.collection(metaCollection).find(sel).toArray(function(err, items) {
                ...
                for (var i: any = 0; i < items.length; i++) {
                    ...
                    var subSel: any = item.cover_medium_url ? { $or: [{ url: Helpers.getUrlByType(item) }, { url: Helpers.getUrlByType(item, 'icon') }, { url: Helpers.getUrlByType(item, 'thumbnail') }, { url: Helpers.getUrlByType(item, 'toc_thumbnail') }] } : { $or: [] }
                    ...
                    s.db.collection('cover').find(subSel).toArray(function(err, items) {
                        ...
                        for (var j: any = 0; j < items.length; j++) {
                            ...
                            var item_type: any = (items[j].url.indexOf(urlTocFile) > 0) ? 'toc_thumbnail' : 'cover';
