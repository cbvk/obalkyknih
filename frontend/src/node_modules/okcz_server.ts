...
class Permissions {
    ...
    static refererValid(referer, ip, encsigla) {
        ...
        if (Permissions.getSiglaForIp(ip)) return Permissions.getSiglaForIp(ip);
    ...
    static ipValid(ip, sigla) {
        //console.log('ipValid:' + ip);
        var tmp: any = Permissions.getSiglaForIp(ip);
...
class Helpers {
    ...
    static replaceUrl(meta: any, bibinfo?: any) {
...
class Server {
    ...
    go(s: Server) {
        ...
        // http://cache1.obalkyknih.cz/api/books/?multi=[{%22isbn%22:%229788376663103%22},{%22isbn%22:%229788376663203%22}]&sigla=CBA001&pretty=1
            ...
            var sigla: any = Permissions.ipValid(this.remoteIP, this.query.sigla);
            ...
            for (var queryKey in this.query) {
                if (queryKey == 'isbn') {
                    var isbns: any = this.query.isbn.split(',');
         ...
         **/
            ...
            if (this.req.method == 'POST') {
                ...
                this.req.on('end', function() {
                    ...
                    var get_data_db: any = { book_id: this.query.book_id, sigla: sigla };
                    if (this.query.id) get_data_db.id = this.query.id;
         ...
         **/
            ...
            if (this.req.method == 'POST') {
                ...
                this.req.on('end', function() {
                    var book_id: any = parseInt(this.query.book_id);
                    if (book_id > 0) {
                        try {
            ...
            request({ url: url, timeout: timeout }, function(error, res, body) {
                if (!error) {
                    ...
                    for (var i: any = 0; i < jsonf.length; i++) {
                        item = jsonf[i];
                        var recId: any = (item.ean || '#') + '-' + (item.oclc || '#') + '-' + (item.nbn || '#');
    ...
    getResult(): Result {
        ...
        if (this.query.isbn) {
            ...
            if (multis.isbn) {
                var isbn: any = multis.isbn.split(' ')[0];
                var value: any = toEan(isbn);
