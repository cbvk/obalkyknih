...
//var urlMain: string = "192.168.1.110" //local

// url frontendu / spolecne url vsech frontnendu
var urlReplace: string = "cache.obalkyknih.cz" //cache1
//var urlReplace: string = "cache2.obalkyknih.cz"  //cache2
//var urlReplace: string = "127.0.0.1"        //local

// IP adresa backendu - udsud je mozne prijimat ridici prikazy
var ipBackend: string = "::ffff:172.30.144.13"        //cache1
//var ipBackend: string = "::ffff:195.113.145.2"         //cache2
//var ipBackend: string = "127.0.0.1"         //local

const urlPart: string = "/"
const urlMetadata: string = "api/books"
...
const urlBibFile: string = "file/bib"
const urlBibApiThumbnail: string = "api/bib/thumbnail"
const urlBibApiPdf: string = "api/bib/pdf"
const uriPartBibThumbnail: string = "/thumbnail"
const uriPartBibPdf: string = "/pdf"
const urlDoporucApi: string = "api/doporuc"
const uriAlive: string = "runtime/alive"
// kolekcia s nastavenimi push API
const push_collection = "settings_push";
// kolekce s metadaty tak jak je pojmenovana v mongodb
const metaCollection: string = "okcz"
const metaAuthCollection: string = "okcz_auth"
const doporucCollection: string = "okcz_doporuc"
...
// adresar s BIB dokumenty
const bibFileDir: string = "/home/users/okcz/static/bib/"
// autority
const urlAuthMeta: string = "api/auth/meta"
const urlAuthMetaBE: string = "api/auth"
const urlAuthParams: string = "?auth="
const urlAuthCover: string = "api/auth/cover"
...
const cryptoPasskey: string = "0kcz,ApIv.3*";
const regexpSigla: any = new RegExp(/^[A-Z0-9]{6}$/);
...
interface Stats {
    ...
    etag_bib_pdf_match: any;
    etag_tib_thumbnail_match: any;
    etag_file_match: any;
    file_requests: any;
    timeout_count: any;
    meta_requests: any;
    ...
    bib_thumbnail_requests: any;
    bib_thumbnail_api_requests: any;
    bib_thumbnail_fetches: any;
    bib_thumbnail_notfound: any;
    bib_thumbnail_removes: any;
    bib_pdf_requests: any;
    bib_pdf_api_requests: any;
    bib_pdf_notfound: any;
    meta_removes: any;
    cover_removes: any;
    apiruntime_requests: any;
    unbound_requests: any;
    operative_requests: any;
    livemap: any;
...
class Statistics {
    ...
    static etagBibPdfMatch: any = {}; // pocet dotazu na BIB PDF, kesovanych prohlizecem od spusteni
    static etagBibThumbMatch: any = {}; // pocet dotazu na BIB nahled, kesovanych prohlizecem od spusteni
    ...
    static bibThumbnailRequests: any = {}; // pocet dotazu BIB nahledu pres urlBibFile thumbnail (/file/toc/xxxxxx/thumbnail) od spusteni
    static bibThumbnailApiRequests: any = {}; // pocet dotazu BIB nahledu pres urlBibApi thumbnail od spusteni
    static bibThumbnailFetches: any = {}; // pocet nacteni BIB nahledu z backendu od spusteni
    static bibThumbnailNotfound: any = {}; // pocet nenalezenych BIB nahledu z backendu od spusteni
    static bibThumbnailRemoves: any = 0; // pocet smazanych BIB nahledu z backendu od spusteni
    static bibPdfRequests: any = {}; // pocet dotazu na BIB dokumenty pres urlBibFile thumbnail (/file/bib/xxxxxx/thumbnail) od spusteni
    static bibPdfApiRequests: any = {}; // pocet dotazu na BIB dokumenty pres urlBibApiThumbnail od spusteni
    static bibPdfNotfound: any = {}; // pocet nenalezenych na BIB dokumenty z backendu od spusteni
    ...
    static apiRuntimeRequests: any = 0; // pocet dotazu na /api/tuntime/alive
    static unboundRequests: any = 0; // vsechny jine dotazy
    static operativeRequests: any = 0; // operativni dotazy (pridani opravneni, komentare/hodnoceni, vsechny ridici requesty mezi BE a FE)

    static dateTimeout: any = 0; // cas do kdy je platny timeout
    static dateStartup: any = new Date(); // doba startu

    static livemap: any = null; // fe live mapy
    ...
    static addEtagBibPdfMatch(sigla: any) {
        this.etagBibPdfMatch[sigla]++ || (this.etagBibPdfMatch[sigla] = 1);
        if (sigla != 'admin') this.etagBibPdfMatch.admin++ || (this.etagBibPdfMatch.admin = 1);
    }

    static addEtagBibThumbMatch(sigla: any) {
        this.etagBibThumbMatch[sigla]++ || (this.etagBibThumbMatch[sigla] = 1);
        if (sigla != 'admin') this.etagBibThumbMatch.admin++ || (this.etagBibThumbMatch.admin = 1);
    }
    ...
    static addCoverRequests(sigla: any) {
        ...
        if (this.livemap !== null && sigla) {
            this.livemap[sigla]++ || (this.livemap[sigla] = 1);
        }
    }
    ...
    static addBibThumbnailRequests(sigla: any) {
        this.bibThumbnailRequests[sigla]++ || (this.bibThumbnailRequests[sigla] = 1);
        if (sigla != 'admin') this.bibThumbnailRequests.admin++ || (this.bibThumbnailRequests.admin = 1);
    }

    static addBibPdfRequests(sigla: any) {
        this.bibPdfRequests[sigla]++ || (this.bibPdfRequests[sigla] = 1);
        if (sigla != 'admin') this.bibPdfRequests.admin++ || (this.bibPdfRequests.admin = 1);
    }
    ...
    static addBibThumbnailApiRequests(sigla: any) {
        this.bibThumbnailApiRequests[sigla]++ || (this.bibThumbnailApiRequests[sigla] = 1);
        if (sigla != 'admin') this.bibThumbnailApiRequests.admin++ || (this.bibThumbnailApiRequests.admin = 1);
    }

    static addBibPdfApiRequests(sigla: any) {
        this.bibPdfApiRequests[sigla]++ || (this.bibPdfApiRequests[sigla] = 1);
        if (sigla != 'admin') this.bibPdfApiRequests.admin++ || (this.bibPdfApiRequests.admin = 1);
    }
    ...
    static addBibThumbnailRemoves() {
        this.bibThumbnailRemoves++;
    }
    ...
    static addMetaFetches(sigla: any) {
        ...
        if (this.livemap !== null && sigla) {
            this.livemap['fetch']++ || (this.livemap['fetch'] = 1);
        }
    }
    ...
    static addBibThumbnailNotfound(sigla: any) {
        this.bibThumbnailNotfound[sigla]++ || (this.bibThumbnailNotfound[sigla] = 1);
        if (sigla != 'admin') this.bibThumbnailNotfound.admin++ || (this.bibThumbnailNotfound.admin = 1);
    }

    static addBibPdfNotfound(sigla: any) {
        this.bibPdfNotfound[sigla]++ || (this.bibPdfNotfound[sigla] = 1);
        if (sigla != 'admin') this.bibPdfNotfound.admin++ || (this.bibPdfNotfound.admin = 1);
    }
    ...
    static addBibThumbnailFetches(sigla: any) {
        this.bibThumbnailFetches[sigla]++ || (this.bibThumbnailFetches[sigla] = 1);
        if (sigla != 'admin') this.bibThumbnailFetches.admin++ || (this.bibThumbnailFetches.admin = 1);
    }

    static addApiAliveRequest() {
        this.apiRuntimeRequests++;
    }

    static addUnboundRequest() {
        this.unboundRequests++;
    }

    static addOperativeRequest() {
        this.operativeRequests++;
    }
    ...
    static addEtagStatisticByEtagPrefix(etagPrefix: string, sigla: any) {
        ...
        } else if (etagPrefix === 'bibp') {
            this.addEtagBibPdfMatch(sigla);
        } else if (etagPrefix === 'bibt') {
            this.addEtagBibThumbMatch(sigla);
        } else {
    ...
    static getStatisticsBySigla(s: any, sigla: number) {
        ...
        stats.etag_bib_pdf_match = this.etagBibPdfMatch[sigla] || 0;
        stats.etag_bib_thumbnail_match = this.etagBibThumbMatch[sigla] || 0;
        ...
        stats.bib_thumbnail_requests = this.bibThumbnailRequests[sigla] || 0;
        stats.bib_thumbnail_api_requests = this.bibThumbnailApiRequests[sigla] || 0;
        stats.bib_thumbnail_fetches = this.bibThumbnailFetches[sigla] || 0;
        stats.bib_thumbnail_notfound = this.bibThumbnailNotfound[sigla] || 0;
        stats.bib_thumbnail_removes = this.bibThumbnailRemoves;
        stats.bib_pdf_requests = this.bibPdfRequests[sigla] || 0;
        stats.bib_pdf_api_requests = this.bibPdfApiRequests[sigla] || 0;
        stats.bib_pdf_notfound = this.bibPdfNotfound[sigla] || 0;
        stats.meta_removes = this.metaRemoves;
        stats.cover_removes = this.coverRemoves;
        stats.apiruntime_requests = this.apiRuntimeRequests || 0;
        stats.unbound_requests = this.unboundRequests || 0;
        stats.operative_requests = this.operativeRequests || 0;
    ...
    static clearData() {
        ...
        this.etagBibPdfMatch = {};
        this.etagBibThumbMatch = {};
        this.etagFileMatch = {};
        this.fileRequests = {};
        ...
        this.tocPdfNotfound = {};
        this.bibThumbnailRequests = {};
        this.bibThumbnailApiRequests = {};
        this.bibThumbnailFetches = {};
        this.bibThumbnailNotfound = {};
        this.bibThumbnailRemoves = 0;
        this.bibPdfRequests = {};
        this.bibPdfApiRequests = {};
        this.bibPdfNotfound = {};
        this.apiRuntimeRequests = 0;
        this.unboundRequests = 0;
        this.operativeRequests = 0;
    }
...
class Permissions {
    ...
    static getSiglaForHeader(headers): string {
        // user agent
        var userAgent: string = headers['user-agent'] || '';
        if (userAgent.substring(0,19) == 'facebookexternalhit')
            return '_FBOOK';

        // library client
        if (!headers['x-library-client'])
            return;

        var libraryClient: string = headers['x-library-client'];
        var librarySigla: string = headers['x-library-code'] || '';

        if (libraryClient.substring(0,5)=='KPSYS') {
            return '_KPSYS';
        }
        else if (libraryClient.substring(0,7).toUpperCase()=='tritius') {
            if (librarySigla.length != 6) librarySigla = 'TRI001';
            return librarySigla.toUpperCase();
        }
        else if (libraryClient.substring(0,7).toUpperCase()=='carmen') {
            if (librarySigla.length != 6) librarySigla = '_CARME';
            return librarySigla.toUpperCase();
        }
        else if (libraryClient.substring(0,7).toUpperCase()=='clavius') {
            if (librarySigla.length != 6) librarySigla = '_CLAVI';
            return librarySigla.toUpperCase();
        }
    }

    static refererValid(referer, ip, encsigla, headers, query) {
        var sigla: string;

        if (encsigla) {
            sigla = Helpers.decrypt(encsigla);
            if (regexpSigla.test(sigla)) return sigla;
        }

        sigla = Permissions.getSiglaForIp(ip);
        if (sigla) return sigla;

        sigla = Permissions.getSiglaForHeader(headers);
        if (sigla) return sigla;

        if (!referer) return false;

        // Clavius
        if (referer.indexOf('l.dll?cll~P') > 0) {
            sigla = query.sigla || ''; // sigla jako parametr
            var prefix: string = query.prefix || '';
            if (sigla == '' && prefix != '') sigla = '!' + prefix + '00000';
            if (sigla.length != 6) sigla = '_CLAVI';
        ...
        // shoda podle wildcardu domenu 3. radu
        var firstSlashes = referer.indexOf('//');
        var firstDot = referer.indexOf('.');
        var subdomain3rd = referer.substr(firstSlashes, firstDot-firstSlashes+1);
        var refererWildcardSubdomain3rd = referer.replace(subdomain3rd, '//*.');
        console.log(refererWildcardSubdomain3rd);
        if (Permissions.referers[refererWildcardSubdomain3rd]) {
            return Permissions.referers[refererWildcardSubdomain3rd];
        }
        if (Permissions.referers[refererWildcardSubdomain3rd + '/']) {
            return Permissions.referers[refererWildcardSubdomain3rd + '/'];
        }
    ...
    static ipValid(xreferer, ip, sigla, headers) {
        //console.log('ipValid:' + ip);
        var tmp: any = Permissions.refererValid(xreferer, ip, null, headers, null);
...
class Helpers {
    ...
    static replaceUrl(meta: any, simple?: any, noocr?: any, bibinfo?: any) {

        // zjednoduseny vystup metadata API
        if (simple !== undefined) {
            var simpleItemsList = ['cover_thumbnail_url', 'cover_icon_url', 'cover_medium_url', 'cover_preview510_url', 'toc_pdf_url', 'toc_thumbnail_url', 'bib_pdf_url', 'bib_thumbnail_url', 'backlink_url', 'book_id'];
        ...
        // odstranit toc_full_text
        if (noocr !== undefined && meta.hasOwnProperty('toc_full_text')) {
            delete meta['toc_full_text'];
            delete meta['bib_full_text'];
        }
        ...
        if (meta.bib_text_url) {
            delete meta.bib_text_url;
        }
        if (meta.bib_thumbnail_url) {
            meta.bib_thumbnail_url = meta.bib_thumbnail_url.replace(urlMain, urlReplace);
        }
        if (meta.bib_pdf_url && meta.bib_pdf_url.indexOf('/view') === -1) {
            meta.bib_pdf_url = meta.bib_pdf_url.replace(urlMain, urlReplace);
        }
        if (bibinfo) {
    ...
    static getUrlByType(metadata: any, type?: any) {
        ...
        switch (type) {
            ...
            case 'toc_thumbnail':
                ...
                break;
            case 'bib_thumbnail':
                url = metadata.bib_thumbnail_url;
                if (!url) return null; // bib_thumbnail nesmi v pripade neexistence vratit cover_medium_url
                break;
            default:
    ...
    static partValidation(p) {
        ...
        Object.keys(p).forEach(function(key) {
            var prop = p[key];
            if (prop.match(/<(.*)>/)) {
                console.log('Err: param "' + key + '":"' + prop + '" contains illegal characters');
                return false;
            }
        });
        return true;
...
class PushSettings {
    static settings: PushSettingsRecord[] = [];

    static removeSettings(server: Server, sigla: any, collection: any) {
    ...
    static modifySettings(server: Server, settings: any, collection: any) {
    ...
    static addSettings(server: Server, settings: any, collection: any) {
        var s: PushSettingsRecord = new PushSettingsRecord(settings);
    ...
    static getPushSettings(clear = false, settings: any, db: any) {
        if (clear === true) {
    ...
    static getSettings(): PushSettingsRecord[] {
        return PushSettings.settings;
    }

    static getSettingsForSigla(sigla: any): PushSettingsRecord {
...
class Server {
    ...
    constructor(req: any, response: any, db: any) {
        ...
        if (!this.referer && this.xreferer) this.referer = this.xreferer;
        // console.log(etags);
        ...
        this.sigla = Permissions.refererValid(this.referer, this.remoteIP, this.encsigla, this.headers, this.query);
    }
    ...
    go(s: Server) {
        ...
        // http://cache.obalkyknih.cz/favicon.ico
            ...
            if (!this.isRequestValid(sigla, true)) {
                this.logUnbound(this);
                Statistics.addUnboundRequest();
                return;
            }
        ...
        /**
         * FILE BIB (thumbnail + pdf)
         **/
        // http://cache.obalkyknih.cz/file/bib/1/thumbnail
        // http://cache.obalkyknih.cz/file/bib/1/pdf
        else if ((this.requrl.indexOf(urlBibFile)) > 0) {
            var sigla: string = this.sigla;
            testLog("41", "[FILE BIB] Sigla:" + sigla);
            ...
            if ((reqUrl.indexOf(uriPartBibThumbnail)) > 0) {
                // BIB thumbnail request
                testLog("44", "->-> BIB thumbnail request");
                Statistics.addBibThumbnailRequests(sigla);
                this.cover(this, reqUrl);
            } else if ((reqUrl.indexOf(uriPartBibPdf)) > 0) {
                // BIB file request
                testLog("44", "->-> BIBBIB pdf file request");
                Statistics.addBibPdfRequests(sigla);
                this.bib(this, reqUrl);
            }
        }
        ...
        // http://cache.obalkyknih.cz/api/cover/?isbn=1210-8480
            ...
            this.db.collection(metaCollection).find(result.sel).toArray(function(err, items_raw) {
                ...
                if (!items_raw.length) {
                    ...
                    s.metadata(s, [{ bibinfo: result.bib, permalink: "a" }], undefined, [], true, false, false);
                    return;
                }
                ...
                if (items_raw) {
                    ...
                    for (var i: number = 0; i < items.length; i++) {
                        ...
                        if (itemInfo.item !== null && s.isGoodItemInfoForBibInfo(itemInfo, bibinfo)) {
                            ...
                            // 0=ISBN, 1=OCLC, 2=NBN, 3=UUID, 4=ISMN, 5=ISBN_OTHER, 6=NBN_OTHER, 7=OCLC_OTHER
        ...
        // http://cache2.obalkyknih.cz/api/toc/thumbnail?isbn=1212-8570
            ...
            this.db.collection(metaCollection).find(result.sel).toArray(function(err, items_raw) {
                ...
                if (!items_raw.length) {
                    // obalka neni v DB a je potrebne nacist z back-end API metadata
                    // funkce metadata() rovnou posle klientovi obalku
                    s.metadata(s, [{ bibinfo: result.bib, permalink: "a" }], undefined, [], false, true, false);
                    return;
                }
        ...
        // http://cache2.obalkyknih.cz/api/toc/pdf?isbn=1212-8570
            ...
            this.db.collection(metaCollection).find(result.sel).toArray(function(err, items_raw) {
                ...
                if (!items_raw.length) {
                    // obalka neni v DB a je potrebne nacist z back-end API metadata
                    // funkce metadata() rovnou posle klientovi obalku
                    s.metadata(s, [{ bibinfo: result.bib, permalink: "a" }], undefined, [], false, false, true);
                    return;
                }
         ...
         * API BIB THUMBNAIL
         **/
        // http://cache2.obalkyknih.cz/api/bib/thumbnail?multi={%22isbn%22:%221212-8570%22}
        // http://cache2.obalkyknih.cz/api/bib/thumbnail?isbn=1212-8570
        else if ((this.requrl.indexOf(urlBibApiThumbnail)) > 0) {
            var sigla: string = this.sigla;
            ...
            Statistics.addBibThumbnailApiRequests(sigla);
            ...
            this.db.collection(metaCollection).find(result.sel).toArray(function(err, items_raw) {
                ...
                if (!items_raw.length) {
                    // obalka neni v DB a je potrebne nacist z back-end API metadata
                    // funkce metadata() rovnou posle klientovi obalku
                    s.metadata(s, [{ bibinfo: result.bib, permalink: "a" }], undefined, [], false, false, false, true, false);
                    return;
                }
                    ...
                    for (var i = 0; i < items.length; i++) {
                        ...
                        if (itemInfo.item !== null && s.isGoodItemInfoForBibInfo(itemInfo, bibinfo)) {
                            ...
                            for (var j: any = 0; j <= 7; j++) {
                                ...
                                if (found) {
                                    var bibThumbnailUrl: any = Helpers.getUrlByType(itemInfo.item, 'bib_thumbnail');

                                    if (s.sendPlaceholderIfNotValue(bibThumbnailUrl)) {
                                        return;
                                    }

                                    s.cover(s, bibThumbnailUrl, false, true);
                                    return;
                                }
                            }
                        }
         ...
         * API BIB PDF
         **/
        // http://cache2.obalkyknih.cz/api/bib/pdf?multi={%22isbn%22:%221212-8570%22}
        // http://cache2.obalkyknih.cz/api/bib/pdf?isbn=1212-8570

        else if ((this.requrl.indexOf(urlBibApiPdf)) > 0) {
            var sigla: string = this.sigla;
            ...
            Statistics.addBibPdfApiRequests(sigla);
            ...
            this.db.collection(metaCollection).find(result.sel).toArray(function(err, items_raw) {
                ...
                if (!items_raw.length) {
                    // obalka neni v DB a je potrebne nacist z back-end API metadata
                    // funkce metadata() rovnou posle klientovi obalku
                    s.metadata(s, [{ bibinfo: result.bib, permalink: "a" }], undefined, [], false, false, false, false, true);
                    return;
                }
                    ...
                    for (var i: number = 0; i < items.length; i++) {
                        ...
                        if (itemInfo.item !== null && s.isGoodItemInfoForBibInfo(itemInfo, bibinfo)) {
                            ...
                            for (var j: number = 0; j <= 7; j++) {
                                ...
                                if (found) {
                                    if (!itemInfo.item.bib_pdf_url) {
                                        s.send404IfNotValue(false);
                                        return;
                                    } else {
                                        s.bib(s, itemInfo.item.bib_pdf_url);
                                        return;
                                    }
        ...
        // http://cache1.obalkyknih.cz/api/books/?multi=[{%22isbn%22:%229788376663103%22},{%22isbn%22:%229788376663203%22}]&sigla=CBA001&pretty=1
            ...
            var sigla: any = Permissions.ipValid(this.xreferer||this.referer, this.remoteIP, this.query.sigla, this.headers);
            ...
            var resNoocr: any = this.query.noocr;
            ...
            this.response.writeHead(200, { 'Content-Type': 'application/json', "Access-Control-Allow-Origin": "*", "Access-Control-Allow-Methods": "GET", "Access-Control-Allow-Headers": "*" });
            var json: any = [];
            ...
            // pole vsech requestu / pozdeji se budou odmazavat requesty, kterych zaznamy se najdou v DB (toto je zaloha pole fetcha)
            let fetchaAll = Object.assign({}, fetcha);


            var dbFind = [];
            if (isbna.length) {
                dbFind.push({ ean: { $in: isbna } });
                dbFind.push({ ean_other: { $in: isbna } });
            }
            if (nbna.length) {
                dbFind.push({ nbn: { $in: nbna } });
                dbFind.push({ nbn_other: { $in: nbna } });
            }
            if (oclca.length) {
                dbFind.push({ oclc: { $in: oclca } });
                dbFind.push({ oclc_other: { $in: oclca } });
            }
            ...
            this.db.collection(metaCollection).find({ $or: dbFind }).toArray(function(err, items_raw) {
                ...
                if (items_raw) {
                    ...
                    for (var i: any = 0; i < items.length; i++) {
                        ...
                        if (itemInfo.item !== null) {
                            // uplatneni priority parametru
                            // 0=ISBN, 1=OCLC, 2=NBN, 3=UUID, 4=ISMN, 5=OTHER ISBN, 6=NBN_OTHER, 7=OCLC_OTHER
                            for (var j: any = 0; j <= 7; j++) {
                                ...
                                for (var k in fetcha) {
                                    ...
                                    if (found) {
                                        ...
                                        var itemToInsert: any = Object.assign({}, Helpers.replaceUrl(itemInfo.item, resSimple, resNoocr));
                                        if (pos >= 0) {
                                            json[pos] = itemToInsert;
                                            delete fetchaStr[pos];
                                        } else {
                                            json.push(itemToInsert);
                                        }
                                        delete fetcha[k];
                                    }
                                }
                ...
                s.metadata(s, fetchaBE, fetchaAll, json);
            });
        }
        ...
        // http://cache1.obalkyknih.cz/api/auth/meta/?id=jk01083016&sigla=CBA001&pretty=1
            ...
            var sigla: any = Permissions.ipValid(this.xreferer, this.remoteIP, this.query.sigla, this.headers);
                        ...
                        if (item !== null) {
                            ...
                            for (var k in fetcha) {
                                ...
                                if (found) {
                                    ...
                                    if (pos >= 0) {
                                        json[pos] = Helpers.replaceUrl(item, resSimple, resNoocr);
                                        delete fetchaStr[pos];
                                    } else {
                                        json.push(Helpers.replaceUrl(item, resSimple, resNoocr));
                                    }
                                    delete fetcha[k];
                                }
                            }
         ...
         **/
            ...
            Statistics.addOperativeRequest();
            var sigla: any = Permissions.ipValid(this.xreferer, this.remoteIP, this.query.sigla, this.headers);
        ...
        /**
         * PRIDANI NOVEHO NASTAVENI PUSH API
         **/
         ...
         **/
            ...
            Statistics.addOperativeRequest();
            let push_collection = this.db.collection('okcz_push');

            var is_post: any = this.req.method == 'POST';
            if (is_post) {
                var post_data: any = '';
                ...
                this.req.on('end', function() {
                    s.syncPushApi(push_collection, post_data, null);
                });
            }

            var sel: any = { $or: [] };
            if (this.query.isbn) {
                var isbn: any = this.query.isbn.split(' ')[0];
                var ean: any = toEan(isbn);
                sel.$or.push({ ean: ean });
            }
            if (this.query.nbn) {
            ...
            if (this.query.ismn) {
                sel.$or.push({ ismn: this.query.ismn });
            }
            if (this.query.oclc) {
            ...
            if (sel.$or.length == 0) {
                ...
                var ids: any = JSON.stringify(sel.$or);
                s.syncPushApi(push_collection, (is_post ? null : ids), ids);
            }
            ...
            this.db.collection(metaCollection).find(sel).toArray(function(err, items) {
                ...
                for (var i: any = 0; i < items.length; i++) {
                    var item: any = items[i];
                    var bookId: any = item.book_id;

                    // dotaz do kolekce cover; hledame obalky a nahledy obsahu na smazani
                    var subSel: any = item.cover_medium_url ? { $or: [{ url: Helpers.getUrlByType(item) }, { url: Helpers.getUrlByType(item, 'icon') }, { url: Helpers.getUrlByType(item, 'thumbnail') }, { url: Helpers.getUrlByType(item, 'preview510') }, { url: Helpers.getUrlByType(item, 'toc_thumbnail') }, { url: Helpers.getUrlByType(item, 'bib_thumbnail') }] } : { $or: [] }

                    // TOC
                    if (item.toc_thumbnail_url) subSel.$or.push({ url: Helpers.getUrlByType(item, 'toc_thumbnail') });
                    var toc: any = item.toc_thumbnail_url
                    var tocPdf: any = item.toc_pdf_url
                    ...
                    // BIB
                    if (item.bib_thumbnail_url) subSel.$or.push({ url: Helpers.getUrlByType(item, 'bib_thumbnail') });
                    var bib: any = item.bib_thumbnail_url
                    var bibPdf: any = item.bib_pdf_url
                    // vymaz i BIB nahled, pokud nejaky je
                    if (bib) {
                        delete etags['bibt-' + md5(bib)];
                    }
                    // vymaz pouze BIB PDF etag; samotny BIB PDF je na disku
                    if (bibPdf) {
                        delete etags['bibp-' + md5(item.bib_pdf_url)];
                    }
                    ...
                    s.db.collection('cover').find(subSel).toArray(function(err, items) {
                        ...
                        for (var j: any = 0; j < items.length; j++) {
                            var cid: any = items[j]._id;
                            var item_type: any = 'cover';
                            if (items[j].url.indexOf(urlTocFile) > 0) item_type = 'toc_thumbnail';
                            if (items[j].url.indexOf(urlBibFile) > 0) item_type = 'bib_thumbnail';
                            (function(cid, item_type, item_url) {
                                s.db.collection('cover').remove({ _id: cid }, { single: true }, function(err, removed) {
                                    if (err) console.log('failed to remove ' + item_type + ' _id: ' + cid);
                                        ...
                                        } else if (item_type == 'bib_thumbnail') {
                                            Statistics.addBibThumbnailRemoves();
                                            delete etags['bibt-' + md5(item_url)];
                                        } else {
                    ...
                    // odstranit doporuceni svazana s mazanou knihou
                    if (bookId) {
                        s.db.collection(doporucCollection).remove({ book_id: bookId });
                    }
                ...
                s.db.collection(metaCollection).remove(sel, function(err, removed) {
                    if (err) s.response.end('Failed to remove _id: ' + item._id);
                    else {
                        s.response.end('ok');
                    }
                });
        ...
        /**
         * ODSTRANENI METADAT AUTORIT (vcetne obrazku)
         **/
        else if (this.query.remove_auth==='true') {
            Statistics.addOperativeRequest();
            if (s.remoteIP === ipBackend) {
                var sel:any = {};
                if (this.query.auth_id) {
                    sel = {auth_id : this.query.auth_id};
                }
                if (sel.length == 0) {
                    s.response.writeHead(404);
                    s.response.end('No identifier');
                    return;
                }
                s.response.writeHead(200);

                s.db.collection(metaAuthCollection).find(sel).toArray(function (err, items) {
                    if (!items) {
                        s.response.end('Wrong request');
                        return;
                    }
                    ...
                    for (var i=0; i<items.length; i++) {
                        ...
                        // dotaz do kolekce cover; hledame obalky a nahledy obsahu na smazani
                        var subSel:any = item.cover_medium_url ? {$or:[{url:Helpers.getUrlByType(item)},{url:Helpers.getUrlByType(item,'icon')},{url:Helpers.getUrlByType(item,'thumbnail')}]} : {$or:[]};
                                ...
                                (function(cid, item_type, item_url) {
                                    s.db.collection('cover').remove({_id:cid},{single:true}, function(err, removed){
                                        if (err) console.log('failed to remove ' + item_type + ' _id: ' + cid);
                                            ...
                                            Statistics.addCoverRemoves();
                                            delete etags[cid];
                                            console.log('removed ' + item_type + ' etag: ' + cid);
                                        }
                                    });
                                })(cid, item_type, items[j].url);
                            }
                        ...
                        Statistics.addMetaRemoves(); // mazani nize maze vice zaznamu na jednou; tato inkrementace se provede pro kazdy metadatovy zaznam jednou
                    }

                    /// smaz vsechna pozadovana metadata
                    console.dir(sel);
                    s.db.collection(metaAuthCollection).remove(sel, function(err, removed){
                        if (err)
            ...
            if (this.req.method == 'POST') {
                ...
                this.req.on('end', function() {
                    ...
                    if (post_data.rating_value === 'like') {
                        ...
                        var rating: any = parseInt(post_data.rating_value) * 10 || 0; // na BE je stupnice 0-100
                    ...
                    var post_data_db: any = {};
                    if (rating > 0) post_data_db.rating = rating;
                    if (post_data.review_text) post_data_db.html_text = post_data.review_text;
                    var get_data_db: any = { book_id: s.query.book_id, sigla: sigla };
                    if (s.query.id) get_data_db.id = s.query.id;

                    s.db.collection('be_sync').insert({ timestamp: s.timestamp, flag_synced: 0, retry_count: 0, retry_date: s.timestamp, get_data: get_data_db, post_data: post_data_db, uri: 'api/add_review' }, { w: 1 }, function(err, result) {
                        s.response.writeHead(200);
                        s.response.end('ok');
                    });
                });
         ...
         **/
            ...
            // zarazeni synchronizacni udalosti do fronty (kolekce be_sync)
            var get_data_db: any = { id: this.query.id, sigla: sigla };
         ...
         **/
            ...
            if (this.req.method == 'POST') {
                ...
                this.req.on('end', function() {
                    var book_id: any = parseInt(s.query.book_id);
                    if (book_id > 0) {
                        try {
                            var post_json: any = JSON.parse(body);
                        } catch (err) {
                            this.send404IfNotValue(false, 'Check query syntax');
                            return;
                        }
                        if (post_json.review && s.query.review_count) {
                            var review_count: any = post_json.review.length;
                            if (review_count != parseInt(s.query.review_count)) {
                                this.send404IfNotValue(false, 'Review count missed match');
                                return;
                            }
                        }

                        s.db.collection(metaCollection).update({ book_id: s.query.book_id }, { $set: { rating_count: s.query.rating_count, rating_sum: s.query.rating_sum, rating_avg100: s.query.rating_avg100, rating_url: s.query.rating_url, rating_avg5: s.query.rating_avg5, reviews: post_json.review } }, { w: 1 }, function(err, result) {
         ...
         * DOPORUC
         **/
        else if ((this.requrl.indexOf(urlDoporucApi)) > 0) {
            proxy.web(this.req, this.response, {
              target: 'http://okczd.obalkyknih.cz/'
            });
        }
        ...
        /**
         * PRE FRONT-END OKCZ LIVE MAPY
         **/
        else if (this.query.livemap === 'true') {
            Statistics.startLivemap();
            Statistics.addOperativeRequest();
            setTimeout(function () {
                var livemapRes = Statistics.getLivemap();
                s.response.setHeader("Access-Control-Allow-Origin", "*");
                s.response.setHeader("Access-Control-Allow-Methods", "GET");
                s.response.setHeader("Access-Control-Allow-Headers", "*");
                s.response.writeHead(200);
                s.response.end(JSON.stringify(livemapRes, null, ''));
                Statistics.stopLivemap();
            }, 3000);
        }
    ...
    metadata(s: Server, fetcha: any, fetchaAll: any, json: any, coverApi?: any, tocThumbApi?: any, tocPdfApi?: any, bibThumbApi?: any, bibPdfApi?: any) {
        ...
        if ((fetcha.length > 0) && (now > Statistics.dateTimeout)) {
            ...
            request({ url: url, timeout: timeout }, function(error, res, body) {
                if (!error) {
                    ...
                    for (var i: any = 0; i < jsonf.length; i++) {
                        item = jsonf[i];
                        var recId: any = (item.ean || '#') + '-' + (item.oclc || '#') + '-' + (item.nbn || '#') + '-' + (item.ismn || '#');
                        ...
                        if (!item.bibinfo || !fetchaStr.length) {
                            ...
                            if (pos >= 0) {
                                json[pos] = Helpers.replaceUrl(item, resSimple, resNoocr, bibinfoOrig[pos]);
                                delete fetchaStr[pos];
                            } else
                                json.push(Helpers.replaceUrl(item, resSimple, resNoocr));
                        }
                ...
                if (coverApi) {
                    ...
                    s.cover(s, coverUrl, coverApi, tocThumbApi, bibThumbApi);
                } else if (tocThumbApi) {
                    var item: any = json[0];
                    ...
                    s.cover(s, coverUrl || tocThumbnailUrl, coverApi, tocThumbApi, bibThumbApi);
                } else if (tocPdfApi) {
                    var item: any = json[0];
                    ...
                    s.toc(s, tocPdfUrl);
                      ...
                      var item: any = json[0];
                      var bibThumbnailUrl: any = Helpers.getUrlByType(item, 'bib_thumbnail');
                      if (s.sendPlaceholderIfNotValue(bibThumbnailUrl)) {
                          return;
                      }
                      s.cover(s, coverUrl || bibThumbnailUrl, coverApi, tocThumbApi, bibThumbApi);
                  } else if (bibPdfApi) {
                      var item: any = json[0];
                      var bibPdfUrl: any = item.bib_pdf_url;
                      if (s.sendPlaceholderIfNotValue(bibPdfUrl)) {
                          return;
                      }
                      s.bib(s, bibPdfUrl);
                  } else {
            ...
            if (coverApi) {
                ...
                Statistics.addBibThumbnailNotfound(sigla);
                s.sendPlaceholderIfNotValue(false);
            } else if (bibPdfApi) {
                Statistics.addBibPdfNotfound(sigla);
                s.sendPlaceholderIfNotValue(false);
            } else {
                // odpoved API s doporucenim dalsi literatury
                if (query.doporuc && query.doporuc == '1') {
                    (async function() {
                        for (var i: any = 0; i < json.length; i++) {
                            var item = json[i];
                            if (!item || !item.book_id) continue;

                            var itemdb:any = await db.collection(doporucCollection).findOne({ 'book_id': item.book_id });
                            if (itemdb && itemdb.notfound < now) itemdb = undefined;

                            // ziskat nova doporuceni
                            if (!itemdb) {
                                // url requestu na okcz
                                const urlOkczdObj: any = fetchaAll[i];
                                delete urlOkczdObj.bibinfo;
                                const urlOkczdUrl: string = JSON.stringify(urlOkczdObj);
                                // polozka, ktora bude vlozena do DB
                                var itemInsert: any = {};
                                // pole ziskane z okczd.obalkyknih.cz
                                var dataOkczd = await s.getData('http://okczd.obalkyknih.cz/api/doporuc?multi=' + urlOkczdUrl);
                                if (!dataOkczd.length) {
                                    itemInsert['notfound'] = now + timeNotfound * 20; // 20dni
                                }
                                itemInsert['book_id'] = item.book_id;
                                itemInsert['okczd'] = dataOkczd;
                                itemdb = { 'okczd': dataOkczd };
                                await db.collection(doporucCollection).deleteMany({ 'book_id': item.book_id });
                                await db.collection(doporucCollection).insertOne(itemInsert);
                            }

                            // dopln doporuceni do vysledneho JSON
                            if (itemdb && itemdb.okczd) {
                                json[i]['doporuc'] = itemdb.okczd;
                            }
                        }
                    ...
                    // odpoved API bez doporuceni dalsi literatury
                    var restmp: any = JSON.stringify(json, null, query.pretty ? ' ' : '');
                    response.end(restmp);
                }
            }
    ...
    cover(s: Server, coverUrl: any, coverApi?: any, tocThumbApi?: any, bibThumbApi?: any) {
        var sigla = s.sigla;
        var req = s.req;
        var db = s.db;
        var response = s.response;
        ...
        db.collection('cover').findOne({ url: coverUrl }, function(err, item) {
            // typ dokumentu, ktery nacitame (cover, toc, nebo bib nahled)
            var coverType: any = 'cover';
            if (tocThumbApi || (coverUrl.indexOf(urlTocFile)) > 0) coverType = 'toc';
            if (bibThumbApi || (coverUrl.indexOf(urlBibFile)) > 0) coverType = 'bib';
            ...
            if ((item !== null) && (!item.notfound)) {
                ...
                } else if (coverType == 'bib') {
                    response.setHeader('Etag', 'bibt-' + md5(coverUrl));
                    etags['bibt-' + md5(coverUrl)] = null;
                }
                ...
                if (coverApi || tocThumbApi || bibThumbApi) {
                    s.sendPlaceholderIfNotValue(false);
                } else {
                    s.send404IfNotValue(false);
                }
                ...
                var request: any = http.request(options, function(res) {
                    ...
                    if (coverType == 'cover') {
                        ...
                        Statistics.addBibThumbnailFetches(sigla);
                        etag = 'bibt-' + md5(coverUrl);
                    }
    ...
    bib(s: Server, reqUrl: any) {
        ...
        testLog("43", "!!!![metadata] sigla: " + sigla + ", req: " + req + ", db: " + db + ", response: " + response + ", remoteIP:" + remoteIP + ", referer: " + referer + ", timestamp: " + timestamp);
        reqUrl = reqUrl.replace('http://' + urlMain, '');
        var bibPdfUrlSegments: any = reqUrl.split('/');
        var bibFileName: any = bibPdfUrlSegments[bibPdfUrlSegments.length - 2];
        var bibExtension: any = bibPdfUrlSegments[bibPdfUrlSegments.length - 1];
        if (!bibFileName) {
            s.send404IfNotValue(false);
        }

        var mimeType: any = 'application/' + (bibExtension || 'pdf');
        var bibFileEncapDir: any = Math.ceil((parseInt(bibFileName) + 1) / 10000) * 10000;
        var bibFilePath: any = bibFileDir + bibFileEncapDir + '/' + bibFileName + '.' + (bibExtension || 'pdf');
        fileSystem.exists(bibFilePath, function(exists) {
            if (exists) {
                var bibFileStat: any = fileSystem.statSync(bibFilePath);
                console.log('bib ' + bibExtension + ': ' + bibFilePath);
                response.writeHead(200, {
                    'Content-Type': mimeType,
                    'Content-Length': bibFileStat.size,
                    'Etag': 'bibp-' + md5(bibFilePath)
                });
                etags['bibp-' + md5(bibFilePath)] = null;
                var readStream: any = fileSystem.createReadStream(bibFilePath);
                readStream.pipe(response);
                db.collection('logs').insert({ sigla: sigla, ip: remoteIP, referer: referer, op: urlBibFile + '/' + bibFileName + uriPartBibPdf, state: 'local', keywords: [], timestamp: timestamp }, { w: 0 });
            } else {
                Statistics.addBibPdfNotfound(sigla);
                s.send404IfNotValue(false);
            }
    ...
    logUnbound(s:any) {
        ...
        try {
            s.db.collection('unbound').insert(toLog);
        } catch(err) {
            console.log('ERR logUnbound: ' + err);
        }
    };
    logReqAll = function(s:any) {
        var toLog = {
            'date': s.date,
            'requrl': s.requrl,
            'remoteIP': s.remoteIP,
        ...
        s.db.collection('req_all').insert(toLog);
    };
    ...
    getResult(): Result {
        ...
        if (this.query.isbn || this.query.ean) {
            ...
            var sel: any = { $or: [{ nbn: value }, { nbn_other: value }] };
            var bib: any = { nbn: value };
            return (!value) ? undefined : { bib: bib, sel: sel };
        }
            ...
            var sel: any = { $or: [{ oclc: value }, { oclc_other: value }] };
            var bib: any = { oclc: value };
            return (!value) ? undefined : { bib: bib, sel: sel };
            ...
            if (multis.oclc) {
                ...
                sel.$or.push({ oclc_other: multis.oclc });
                bib.oclc = multis.oclc;
            }
            if (multis.nbn) {
                sel.$or.push({ nbn: multis.nbn });
                sel.$or.push({ nbn_other: multis.nbn });
                bib.nbn = multis.nbn;
            }
    ...
    getSortedItemsFromItemsRaw(items_raw: any): any {
        ...
        for (var i = 0; i < items_raw.length; i++) {
            var tmp_part_root: any = items_raw[i].part_root || '0';
    ...
    getItemInfoFromItem(item: any): ItemInfo {
        ...
        itemInfo.ean_other = item.ean_other;
        itemInfo.nbn_other = item.nbn_other;
        itemInfo.oclc_other = item.oclc_other;
    ...
    itemFound(itemInfo: ItemInfo, bibinfo: any, j: number): boolean {
        if (j == 0 && bibinfo.isbn) {
            ...
            if (!isbnBib || !itemInfo.ean) return false;
            if (isbnBib != itemInfo.ean) return false;
            if (itemInfo.part_root == '0' && itemInfo.part_ean_standalone == '0' && !(bibinfo.part_no || bibinfo.part_name || bibinfo.part_year || bibinfo.part_volume)) return false;
            return true;
            ...
            if (itemInfo.ean_other) ean_other_length = itemInfo.ean_other.length;
            for (var i: number = 0; i < ean_other_length; i++) {
                var item = itemInfo.ean_other[i];
                if (!isbnBib || !item) return false;
                if (isbnBib != item) return false;
                if (itemInfo.part_root == '0' && itemInfo.part_ean_standalone == '0' && !(bibinfo.part_no || bibinfo.part_name || bibinfo.part_year || bibinfo.part_volume)) return false;
                return true;
            }
          } else if (j == 6 && bibinfo.nbn) {
              // OTHER NBN
              var nbn_other_length: number = 0;
              if (itemInfo.nbn_other) nbn_other_length = itemInfo.nbn_other.length;
              for (var i: number = 0; i < nbn_other_length; i++) {
                  var item = itemInfo.nbn_other[i];
                  if (!bibinfo.nbn || !item) return false;
                  if (bibinfo.nbn != item) return false;
                  if (itemInfo.part_root == '0' && itemInfo.part_nbn_standalone == '0' && !(bibinfo.part_no || bibinfo.part_name || bibinfo.part_year || bibinfo.part_volume)) return false;
                  return true;
              }
          } else if (j == 7 && bibinfo.oclc) {
              // OTHER OCLC
              var oclc_other_length: number = 0;
              if (itemInfo.oclc_other) oclc_other_length = itemInfo.oclc_other.length;
              for (var i: number = 0; i < oclc_other_length; i++) {
                  var item = itemInfo.oclc_other[i];
                  if (!bibinfo.oclc || !itemInfo.oclc) return false;
                  if (bibinfo.oclc != itemInfo.oclc) return false;
                  if (itemInfo.part_root == '0' && itemInfo.part_oclc_standalone == '0' && !(bibinfo.part_no || bibinfo.part_name || bibinfo.part_year || bibinfo.part_volume)) return false;
                  return true;
    ...
    syncPushApi(collection: any, rec: any, ids: any): any {
        if (!rec && !ids) {
        ...
        let records = [];
        let all_settings: PushSettingsRecord[] = PushSettings.getSettings();

        for (var s in all_settings) {
            let settings = all_settings[s];
            ...
            if (!settings.full_container && ids) {
                var idsSync = { metadata: {} };
                var idsParsed = JSON.parse(decodeURIComponent(ids));
                for (var idKey in idsParsed) {
                    var idValue = idsParsed[idKey];
                    Object.assign(idsSync.metadata, idValue);
                }
                records.push({ sigla: settings.sigla, rec: JSON.stringify(idsSync), retry_count: 0, retry_date: new Date().toISOString() });
            }
        }
