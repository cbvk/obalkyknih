...
function createCitation(s, bibliography, marc) {
    ...
    if (bibliography) {
        ...
        if (s.send404IfNotValue(citace_Settings, 'Unknown citation source settings. You need to sign up at http://www.obalkyknih.cz, provide your catalog URL and setup citation source settings')) {
            return null;
        }
        else {
...
var Marc = (function () {
    ...
    Marc.prototype.GetCitation = function () {
        ...
        console.log("jine_id: " + this.GetValue('024', 'a'));
        console.log("jine_id_typ: " + this.GetValue('024', '2'));
        ...
        if (rok_vydani && rok_vydani.indexOf(',') == -1) {
            if (rok_vydani && rok_vydani[0] == '[' && rok_vydani.indexOf(',') == -1) {
                rok_vydani = rok_vydani.substr(1, rok_vydani.length - 1);
            }
            if (rok_vydani[rok_vydani.length - 1] == ']' && rok_vydani.indexOf(',') == -1) {
                rok_vydani = rok_vydani.substr(0, rok_vydani.length - 1);
            }
            if (rok_vydani) {
                var match = /(\d+)/.exec(rok_vydani);
                if (match)
                    rok_vydani = match[1];
            }
        }
        if (!rok_vydani && this.GetValue(773, 'g')) {
            ...
            // 23.10.2017 - Re: cbvk - citace clanku - cs32300 - normal
            if (rok_vydani != "") rok_vydani = rok_vydani + ", " + this.GetValue(773, 'g');
        }
        ...
        var ISBN = this.GetValue('020', 'a') ? this.GetValue('020', 'a') + "" : "";
        if (ISBN != "") {
        ...
        var ISSN = this.GetValue('022', 'a') ? this.GetValue('022', 'a') : this.GetValue(773, 'x');
        ISSN = ISSN ? "ISSN " + ISSN + ". " : "";
        ...
        if (svazek) {
            ...
            svazek = "";
        var rocnik = svazek;
        var jine_id = this.GetValue('024', 'a');
        var jine_id_typ = this.GetValue('024', '2');
        ...
        switch (this.Type) {
            case "aa":
                ...
                return primarni_odpovednost + nazev_podnazev + "<i>" + nazev_casopisu + "</i>" + vydani + misto_nakladatelstvi_rok +
                    (rocnik_rocnik_cislo != ""
                        ? (misto_nakladatelstvi_rok != "" ? ", " : "") + rocnik_rocnik_cislo
                        : "") +
                    (rozsah_stran != ""
                        ? (misto_nakladatelstvi_rok != "" || rocnik_rocnik_cislo != "" ? ", " + rozsah_stran : rozsah_stran)
                        : (misto_nakladatelstvi_rok != "" || rocnik_rocnik_cislo != "" ? ". " : "")) +
                    DOI + ISSN + (URL ? "Dostupné také z: " + URL + (poznamka ? ". " : "") : "") + (poznamka ? poznamka + "." : "");
            case "em":
            ...
            case "cs":
                return "<i>" + nazev_podnazev + "</i>" + vydani + misto_nakladatelstvi_rok + (rocnik_rocnik_cislo != "" ? (misto_nakladatelstvi_rok != "" ? ", " : "") + rocnik_rocnik_cislo : "") + (pocet_stran != "" ? (misto_nakladatelstvi_rok != "" || rocnik_rocnik_cislo != "" ? ", " : "") + pocet_stran : (misto_nakladatelstvi_rok != "" || rocnik_rocnik_cislo != "" ? ". " : "")) + ISBN + ISSN + jine_id_typ_jine_id + (URL ? "Dostupné také z URL: " + URL : "");
            case "fm":
                var _index = nazev_podnazev.length - 2;
                nazev_podnazev = "<i>" + nazev_podnazev + "</i>";
            ...
            case "ms":
            case "ks":
                return "<i>" + nazev_podnazev + "</i>" + vydani + misto_nakladatelstvi_rok + (rocnik_rocnik_cislo != "" ? (misto_nakladatelstvi_rok != "" ? ", " + rocnik_rocnik_cislo : rocnik_rocnik_cislo) : "") + (pocet_stran != "" ? (misto_nakladatelstvi_rok != "" || rocnik_rocnik_cislo != "" ? ", " : "") + pocet_stran : (misto_nakladatelstvi_rok != "" || rocnik_rocnik_cislo != "" ? ". " : "")) + ISSN + jine_id_typ_jine_id + (URL ? "Dostupné také z URL: " + URL : "");
            case "am":
            case "jm":
            case "cm":
            case "au":
            ...
            case "tm":
            case "rm":
                return primarni_odpovednost + korporace + "<i>" + nazev_podnazev + "</i>" + svazek + vydani + sekundarni_odpovednost + misto_nakladatelstvi_rok + (pocet_stran != "" ? (misto_nakladatelstvi_rok != "" ? ", " + pocet_stran : pocet_stran) : (misto_nakladatelstvi_rok != "" ? ". " : "")) + edice_cislo_edice + ISBN + jine_id_typ_jine_id + (URL ? "Dostupné také z URL: " + URL : "");
            default:
                return "";
        }
    };
    ...
    Marc.prototype.GetIntroAuthorsString = function () {
    ...
    Marc.prototype.GetStringForSecondaryAuthorities = function (authorTag, actionText) {
        ...
        if (authors.length > 0) {
            ...
            text = actionText + (authors[0].indexOf('ová,') > -1 ? 'a' : '') + " " + (nameMatch[1] ? nameMatch[2] + " " + nameMatch[1].toUpperCase() : nameMatch[3]);
            text = text.replace('ditora', 'ditorka');
            text = text.replace('utor úvodua', 'utorka úvodu');
            if (authors.length > 1)
                for (var i = 1; i < authors.length; i++) {
                    nameMatch = /([^,]*), (.*[a-zA-Z].*)|(.*)/.exec(authors[i]);
                    text = text + ", " + actionText + (authors[i].indexOf('ová,') > -1 ? 'a' : '') + " " + (nameMatch[1] ? nameMatch[2] + " " + nameMatch[1].toUpperCase() : nameMatch[3]);
                    text = text.replace('ditora', 'ditorka');
                    text = text.replace('utor úvodua', 'utorka úvodu');
                }
        }
