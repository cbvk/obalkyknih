...
namespace SobekCM.Bib_Package.MARC
{
    public class MARC_Record_Z3950_Retriever
    {
        ...
        public static IEnumerable<MARC_Record> Get_Record(int Attribute_Number, string Search_Term, Z3950_Endpoint Z3950_Server, out string Message, Record_Character_Encoding encoding)
        {
            // Initially set the message to empty
            Message = String.Empty;
            ...
            try
            {
                ...
                //	capture the byte stream
                //record = records[0];
                List<MARC_Record> marcRecordList = new List<MARC_Record>();
                for (int i = 0; i < records.Count; i++ )
                {
                    MemoryStream ms = new MemoryStream(records[i].Content);
